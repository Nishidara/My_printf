...
int gain_space(int a, double nb, double c, char *str)
{
    if (nb / c)
    return (a);
}

int suite_float(int a, double nb, double c, char *str)
{
    for (nb / c; nb == 0 && c == 0;) {
...
int suite_float2(double nb, double c, char* str)
{
    int a = 0;
    int b = 1000000;
    long princ = nb;
...
void my_put_float(int a, double nb, double c, char* str)
{
    if (nb / c){
        if (nb == 0 && c <= 0){
        suite_float(a, nb, c, str);
        } else {
        suite_float2(nb, c, str);
        }
    } else {
        suite_float2(nb, c, str);
    }
